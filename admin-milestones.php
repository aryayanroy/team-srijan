<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        $image = time();
        $upload = upload($_FILES["image"]["tmp_name"], $image, "milestones");
        if($upload == true){
            $sql = $conn->prepare("INSERT INTO milestones (image, title, body, link) VALUES (?, ?, ?, ?)");
    ...
    if(isset($_GET["delete"])){
        $response = array("status" => false, "message" => "No response");
        $sql = $conn->prepare("DELETE FROM milestones WHERE id = ?");
                ...
                <div class="col-9">
                    <article class="p-3 border bg-white rounded">
                        ...
                        <nav class="nav nav-underline nav-fill">
                            <a href="admin-milestones" class="nav-link active">Milestones</a>
                        ...
                        <nav class="nav nav-underline nav-fill mt-2">
                            <a href="milestone" class="nav-link">Basic</a>
                            <a href="admin-milestones" class="nav-link active">Milestones</a>
                        </nav>
                        <div class="row my-3">
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <?php
                                ...
                                $sql = $conn->prepare("SELECT * FROM milestones ORDER BY id DESC LIMIT ?, 10");
                                ...
                                try{
                                    ...
                                    if($sql->rowCount()>0){
                                        ...
                                        echo "<tr><td colspan='6' class='text-center'>No updates for now.</td><tr>";
                                    }
                                }catch(PDOException $e){
            ...
            <div class="modal-content">
                ...
                <form action="<?php echo $_SERVER["PHP_SELF"];?>" method="post" enctype="multipart/form-data">
                    ...
                    <div class="modal-footer">
                        <button type="submit" class="btn btn-primary flex-grow-1">Save milestone</button>
                    </div>
                </form>
            </div>
        </div>
